  ...
  return (
    <div>
      ...
      <div className={`text-white ${navOpen} fixed justify-center flex flex-col h-full transform transition-all duration-500
        delay-300 w-[70%] sm:w-[60%] bg-cyan-800 space-y-6 z-[1000050] right-0`}>
        {NavLinks.map((link) => {
        ...
        {/* Close Icon */}
        <MdCloseFullscreen onClick={closeNav}  className='absolute top-[0.7rem] right-[1.4rem] sm:w-8 sm:h-8 w-6 h-6'/>
      </div>
    </div>
  )
}
